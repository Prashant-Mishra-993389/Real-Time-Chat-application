...
import { savePrivateKey } from '../utils/db';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch('/api/auth/signup', {
        ...
        body: JSON.stringify({ username, email, password, publicKey: publicKeyPem }),
      });
      ...
      await savePrivateKey(data.user.id, privateKeyPem);
      ...
      >
        <div className="text-center mb-10">
          <motion.div 
            ...
            className="w-16 h-16 bg-purple-500/20 rounded-2xl flex items-center justify-center mx-auto mb-6 border border-purple-500/30 shadow-[0_0_20px_rgba(168,85,247,0.4)]"
          >
          ...
          <p className="text-zinc-400 text-sm">
            We'll generate a unique encryption key just for you.
          </p>
        </div>
        ...
        <form className="space-y-5" onSubmit={handleSubmit}>
          ...
          <motion.button
            ...
            type="submit"
            disabled={loading}
            className="group relative w-full flex justify-center items-center gap-2 py-3 px-4 border border-transparent text-sm font-semibold rounded-xl text-white bg-purple-600 hover:bg-purple-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500 focus:ring-offset-[#0a0a0a] disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 shadow-[0_0_15px_rgba(168,85,247,0.5)] hover:shadow-[0_0_25px_rgba(168,85,247,0.6)]"
          >
